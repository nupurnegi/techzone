...
collectDeployments <- function(region, API_KEY, target_tag=NULL) {
  ...
  authResponse = httr::content(httr::POST(url = auth_url, httr::add_headers(.headers=headers), body = data, encode="form"))
  
  
  
  if("access_token" %in% names(authResponse)) {
  ...
  wml_base_url=paste0(paste0("https://",region),".ml.cloud.ibm.com")
  deployment_url=paste0(paste0(wml_base_url,"/ml/v4/deployments?version="),Sys.Date())
